...
void use_index_initialize() noexcept
{
    ...
    // prepare the (shared) pretopn plan & scan plan
    {
        ...
        mktid_date_sorted_flag = new bool[g_mktid_count];
        memset(mktid_date_sorted_flag, 0, g_mktid_count * sizeof(bool));
...
void fn_pretopn_thread_use_index([[maybe_unused]] const uint32_t tid) noexcept
{
    ...
    while (true) {
        ...
        if (ref_result.size() < ref_query.q_topn) {
            DEBUG("warning: pretopn%u finished shared_q_range[%u,%u), ref_qid%u result unfilled(%u<%u)",
                tid, d_range.d_begin, d_range.d_end, max_q_topn_qid, ref_result.size(), ref_query.q_topn);
        }
...
void fn_worker_thread_use_index([[maybe_unused]] const uint32_t tid) noexcept
{
    ...
    while (true) {
        ...
        query_t& query = g_queries[qid];
#if CONFIG_TOPN_DATES_PER_PLATE > 0
        DEBUG("worker%u start to wait for pretopn query%u task_id%u", tid, qid, task_id);
        g_pretopn_queries_done[qid].wait_done();
        DEBUG("worker%u finish wait for pretopn query%u task_id%u", tid, qid, task_id);
#endif
        ...
        while (di_range_queue.pop(&di_range)) {
            DEBUG("worker%u fetch di_range[orderdate=%u,d_offset=%u], qid=%u", tid, 
                di_range.orderdate, di_range.d_offset, qid);
                ...
                for (const uint32_t* p = p_scan_begin; p < p_scan_end; ++p) {
                    ...
                    if (value & 0x80000000) {  // This is orderkey
                        ...
                        if (shipdate > query.q_shipdate) {
                            ...
                            ASSERT(expend_cent > 0, "worker%u query%u orderdate%u off%u pair[%p,%u) p%p",
                                tid, qid, orderdate, off, p_scan_begin, item_count, p);
    ...
    while (true) {
        ...
        const auto mmap_date_item_range = [&](const date_t d_begin, const date_t d_end) {
            ...
            for (date_t orderdate = d_begin; orderdate < d_end; ++orderdate, ++d_offset) {
                DEBUG("loader%u pread orderdate%u, d_offset=%u, qid=%u", 
                    tid, orderdate, d_offset, qid);
